...
- `manual` items stay in the catalog for pack resolution and reporting, but CHEF only prints their source URLs for now
...
- `chef install` installs bundled Codex skills from the resolved item set and reports manual items that still need external setup
