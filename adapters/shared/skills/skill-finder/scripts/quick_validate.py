...
def validator_candidates() -> list[Path]:
    ...
    override = os.environ.get("SKILL_FINDER_QUICK_VALIDATE")
    if override:
...
def find_validator() -> Path:
    ...
    raise SystemExit(
        ...
        f"{searched}\n"
        "Set SKILL_FINDER_QUICK_VALIDATE or CODEX_HOME if needed.",
    )


def runtime_candidates() -> list[Path]:
    candidates: list[Path] = []
    override = os.environ.get("SKILL_FINDER_VALIDATOR_PYTHON")
    if override:
        candidates.append(Path(override).expanduser())
...
def choose_runtime(validator: Path) -> Path:
    ...
    raise SystemExit(
        ...
        f"Searched:\n{searched}\n"
        "Install PyYAML or set SKILL_FINDER_VALIDATOR_PYTHON.",
    )
