...
def default_mirror() -> Path | None:
    override = os.environ.get("SKILL_FINDER_MIRROR")
    if override:
    ...
    standalone_candidate = repository.parent / "skill-finder" / "skill-finder"
    chef_candidate = repository.parent / "chef" / "adapters" / "shared" / "skills" / "skill-finder"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Check or sync a Skill Finder mirror directory.",
    )
    parser.add_argument(
        "mirror",
        nargs="?",
        help=(
            "Mirror skill directory path. Defaults to SKILL_FINDER_MIRROR "
            "or a sibling Chef/skill-finder checkout."
        ),
    )
    parser.add_argument(
        "--mode",
    ...
    if mirror is None:
        raise SystemExit(
            "mirror path required; pass one explicitly or set SKILL_FINDER_MIRROR",
        )
